...
# path_dir = "/data/jaswanth/OM-Gaussian-Splatting-org/output/replica/office_2_with_mlp_triplet_loss_3d_los_2_grads/test/ours_30000"
# # Path to the folders containing the ground truth masks and predicted masks
# gt_folder = "/data/jaswanth/OM-Gaussian-Splatting-org/data/replica/office_2/rs_semantics"
def replace_ids_with_gt(path_dir,gt_folder):
    pred_folder = path_dir+"/pred_semantics"
    output_folder =path_dir+ "/new_pred_semantics"
    vis_out_folder = path_dir+"/new_vis_semantics"
    vis_gt_folder = path_dir+"/gt_vis_semantics"
    os.makedirs(output_folder,exist_ok=True)
    os.makedirs(vis_out_folder,exist_ok=True)
    os.makedirs(vis_gt_folder,exist_ok=True)
    # Iterate over each ground truth mask
    for i,gt_mask_file in enumerate( tqdm(os.listdir(gt_folder),desc="Matching ids")):
